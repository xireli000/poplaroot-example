...
public class SecondaryEnhance implements SecondaryAspectEnhancerManager {
    ...
    public void methodBeforeInvoke(PlugDefinitionInfo target, Method method, Object[] args) {
        System.out.println("准备执行:"+"--SecondaryEnhance--methodBeforeInvoke");
    }
    ...
    public CircuitBreakerInfo aspectCircuitBreaker(Object[] args, String methodName, String methodDescriptor, boolean isVoid, PlugDefinitionInfo target) {
        System.out.println("SecondaryEnhance--参数："+args[0]);
    ...
    public void methodAfterInvoke(PlugDefinitionInfo target, Method method, Object returnValue, Throwable throwable) {
        if (throwable != null) {
            System.out.println("SecondaryEnhance--"+method.getName()+"方法执行出现异常：" + throwable.getMessage());
        } else {
            System.out.println("SecondaryEnhance--"+method.getName()+"方法执行完毕，返回值：" + returnValue);
        }
    }
}
